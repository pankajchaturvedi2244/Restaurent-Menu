  ...
  const handleAddDish = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/dishes', {
        ...
        body: JSON.stringify({
          ...dishForm,
          spiceLevel: dishForm.spiceLevel ? parseInt(dishForm.spiceLevel) : null,
          restaurantId,
      ...
      setDishForm({ name: '', description: '', image: '', spiceLevel: '', categories: [] });
      ...
      <header className="bg-white shadow-md sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex justify-between items-center">
            <div>
              <Link href="/dashboard" className="text-blue-600 hover:underline font-semibold text-sm flex items-center gap-1">
                ← Back to Dashboard
              </Link>
              <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mt-2">Menu Management</h1>
            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {categories.map((category) => (
                      ...
                      <div className="bg-white rounded-lg shadow hover:shadow-lg transition p-6 border-l-4 border-pink-600">
                        ...
                        <h3 className="text-xl font-bold text-gray-900 mb-4">🍽️ {category.name}</h3>
                        <button
              ...
              <div>
                {!showDishForm ? (
                  ...
                  <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
                    <h2 className="text-2xl font-bold text-gray-900 mb-6">Create New Dish</h2>
                    ...
                    <form onSubmit={handleAddDish}>
                      ...
                      <div className="mb-6">
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          <span className="text-2xl">
                            {dishForm.spiceLevel && dishForm.spiceLevel !== '' ? '🌶️'.repeat(parseInt(dishForm.spiceLevel)) : ''}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="mb-6">
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto bg-pink-50 p-4 rounded-lg border-2 border-pink-300">
                          {categories.length === 0 ? (
                            <p className="text-gray-600 text-sm">Create categories first</p>
                          ) : (
                            categories.map((category) => (
                              <label key={category.id} className="flex items-center hover:bg-white p-2 rounded transition">
                                <input
                                  type="checkbox"
                                  checked={dishForm.categories.includes(category.id)}
                                  onChange={() => toggleDishCategory(category.id)}
                                  className="w-5 h-5 text-pink-600 rounded cursor-pointer"
                                />
                                <span className="ml-3 text-gray-700 font-medium cursor-pointer">{category.name}</span>
                              </label>
                            ))
                          )}
                        </div>
                      ...
                      <div className="flex gap-3">
                        <button
                          type="submit"
                          disabled={isSubmitting || dishForm.categories.length === 0}
                ...
                {dishes.length === 0 ? (
                  <div className="bg-white rounded-xl shadow-lg p-12 text-center">
                    <div className="text-5xl mb-4">🍽️</div>
                    <p className="text-gray-600 mb-4">No dishes yet. Create one to get started!</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {dishes.map((dish) => (
                      <div key={dish.id} className="bg-white rounded-lg shadow hover:shadow-lg transition p-6 flex gap-4">
